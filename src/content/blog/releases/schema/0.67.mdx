...
const MyBrand = Schema.Number.pipe(Schema.between(1, 10), Schema.brand("MyNumber"))
...
const Positive = Schema.Number.pipe(Schema.filter((n) => (n > 0 ? undefined : "must be positive")))
...
```ts
console.log(JSON.stringify(JSONSchema.make(Schema.encodedSchema(MySchema)), null, 2))
```
