...
async function getTodo(
  id: number,
): Promise<{ ok: true; todo: Todo } | { ok: false; error: "InvalidJson" | "RequestFailed" }> {
  try {
    const response = await fetch(`/todos/${id}`)
...
export const getTodo = (
  id: number,
): Effect.Effect<unknown, HttpClientError.HttpClientError | Cause.TimeoutException, never> =>
...
const httpCall = (id: number) =>
  HttpClientRequest.get(`/todos/${id}`).pipe(HttpClient.fetchOk, HttpClientResponse.json)
