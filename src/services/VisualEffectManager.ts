...
>()("VisualEffectManager", {
  make: Effect.gen(function* () {
    ...
    const visualTracer = Tracer.make({
      span(options) {
    ...
    const start = <A, E>(
      ...
      effect.pipe(
        Effect.withSpan(options.label, { attributes: { [ATTR_KIND]: PROGRAM_KIND } }),
        Effect.annotateSpans(ATTR_EXAMPLE_KEY, options.key),
        Effect.withTracer(visualTracer),
